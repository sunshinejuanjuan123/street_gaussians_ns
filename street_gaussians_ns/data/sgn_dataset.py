...
class InputDataset(Dataset):
    ...
    def get_data(
        ...
        if segment_filenames := self._dataparser_outputs.metadata.get("segment_filenames"):
            segment_filename = segment_filenames[image_idx]
            data["semantic"], _ = get_semantics_and_mask_tensors_from_path(
                filepath=segment_filename, mask_indices=[-1], scale_factor=self.scale_factor
            )
            assert (
